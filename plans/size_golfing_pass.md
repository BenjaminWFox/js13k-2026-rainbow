...
| Area | File | LOC | Notes |
|------|------|----:|-------|
| World / pipes | [src/pipes.ts](../src/pipes.ts) | 791 | Largest file. Snake walker, occupancy grid, preview/push duplication. Kit bake + accent elbows are load-bearing. |
| Combat | [src/combat.ts](../src/combat.ts) | 779 | Three near-miss implementations of the 6 shared powers (player / miniboss / finale) plus unique horn-chevron math. |
| Swarm | [src/enemies.ts](../src/enemies.ts) | 780 | Spawn/teleport/cap, spatial-hash `separate()`, three copy-pasted ~17-field spawn literals. |
| Cutscene | [src/cutscene.ts](../src/cutscene.ts) | 379 | Choreographed portal/boss/march/drain. Only consumer of `greyPipeCanvas` / grey twin kits. |
| UI / meta | overlays 395 + ui 234 + stats 242 | ~871 | Menus, 11 shop rows, card copy. |
| Audio (keep as a system) | zzfx + zzfxm + music + audio | ~431 | Prior measurement: **~1,372 B packed** (music ~533, synth+SFX ~839). Re-measure. |
...
| Font: drop `?`; `.` / `-` maybe; `/` maybe | both | **`+` used** (heal FX). **`/` used** (`0/3` shop lines). **`'` used** (`IT'S`). **`!` and `,` used**. **`J` and `?` unused** in current baked/drawn strings. `.` and `-` look unused — re-scan all `bakeText`/`drawText` sites immediately before dropping. Glyph ints compress well; savings may be ~0. |
...
| `LUCK_FOURTH` / `LUCK_FIFTH` → `luck*.25` / `luck*.2` | Fable | **Math checks out:** `[0,0.25,0.5,0.75]` and `[0,0.2,0.4,0.6]`. |
...
- **Drop `pixelScale` from bake** in [src/sprites.ts](../src/sprites.ts). Nested `px/py` loops, struct field, and default arg are unused generality. Expected: identical sprites.
- **Strip unused ZzFX tremolo** in [src/zzfx.ts](../src/zzfx.ts) (param 19 and the `repeatTime ? 1 - tremolo + …` multiply). **Keep the filter** (SUCCESS / PICKUP / HIT). Keep v1.3.2 otherwise (waveforms 0–5 are used). Expected: identical audio.
- Tiny: unused font glyphs in [src/font.ts](../src/font.ts) `CHARS`/`GLYPHS` — only after a fresh string audit. Candidate drops: `J`, `?`; maybe `.` and `-`. **Do not drop `+` `/` `'` `!` `,`**. Measure; savings may be ~0.
...
1. `mkEnemy(overrides)` / `makeEnemy(x, y, extra)` for the three spawn sites — **bulk OK** if every field matches today’s literals (`bobTime` random only on regulars, miniboss `color: i`, finale `FINAL_HP` + `chasing: true`, etc.).
2. `tryBolt(...)` replacing `tryFireball`/`tryFrostball` — **bulk OK** if args preserve targeting, damage, freeze, friendliness.
3. Shared cooldown ticker taking a timer slot — **one-at-a-time** (finale vs player fail-to-fire `cd = 0` behavior).
4. Per-color action table for miniboss + finale — **one-at-a-time**. Preserve yellow skip, chasing gate, finale stagger, full-HP heal skip.
5. Do **not** fold player scaling into the AI table. Player still uses `kitDamage` / `colorDamage` / `powerAmount`; AI stays on base constants.
6. `strokeRing` / `drawRing` / `drawShield` — **one-at-a-time** (visual). Folding `stompFlash` into `novas` as a white nova is the same bucket.
7. Axis-move helper for knockback/chase wall tests — **optional**. Identical copies; may not pay (Cluster A evidence). If attempted, one-at-a-time (movement).
8. Pack `EnemyType` bake into positional arrays / formula `hp`/`radius`/`dmg` — **one-at-a-time** (hitboxes and HP). Formulas today: `hp = 8 + tier*4`, `contactDamage = tier+1`, `radius = max(w,h)/2`.
...
- Merge `straightPreview`+`pushStraight` and `curvePreview`+`pushCurve` into placer functions (preview = dry run, push = commit).
- Replace the 8-row `cornerDefs` table with the **corrected** generator (see verification table). Live-test all 7 pipes for accents and collision.
- Direction lookup for `pushCap` offsets and `portalFromCell` edge branches.
- Pack atlas rects (`CAP`, `STRAIGHT`, `CURVE_*`, `PORTAL_*`) as flat arrays; loop cap/portal bakes.
- Replace `greyTwin: Map` with a parallel array keyed by `kitCanvasList` order — **skip this if Track 2 static-cutscene is likely**, because that cut deletes grey twins entirely.
...
- [src/stats.ts](../src/stats.ts): draft cards as positional tuples instead of `{kind,id,title,body}` **if** the UI still receives the same titles/bodies; single packed `[name, cost]` shop table (`SHOP_NAME` + `SHOP_COST` already parallel); luck formulas `luck*0.25` / `luck*0.2`. **No string shortening** here (that is Track 2).
- [src/audio.ts](../src/audio.ts): one exported `play(name)` over a params table instead of 6 wrappers; merge duplicated suspended-resume paths (`unlockAudio` vs `play`). Same SFX.
- [src/music.ts](../src/music.ts): factor drum+bass rows repeated 6× (`const d=[...], b=[...]`; patterns become `[melody, d, b]`). **Zero audible change** if rows stay byte-identical.
- [src/pickups.ts](../src/pickups.ts): `add(x, y, kind)` with a size table. Preserve scatter: boss guaranteed scrap uses ±14; `dropLoot` uses ±4.
- [src/overlays.ts](../src/overlays.ts): merge `openDeath` / `openWin` (identical except title). Tiny.
- [src/index.ts](../src/index.ts): reuse a `cam` object in `cameraOrigin` (already called twice per frame). No visual change if clamp math is unchanged.

**One-at-a-time (easy to change the game):**

- [src/player.ts](../src/player.ts) axis-generalize the 4-way collision snap (~192–291): one `snap(axis, dir)` + `edgeMin`/`edgeMax` replacing string-tagged `min/maxOverlappingTileEdge`. Wall sliding is gameplay.
- [src/ui.ts](../src/ui.ts): merge `openMenu`/`openCards`; move title-screen-only scale/top-placement (`titleScale`, `headingTop`) out of `layoutUi`. Title placement is visual.
- [src/cutscene.ts](../src/cutscene.ts): pre-break `SCRIPT` / delete `wrap()`; loop the duplicated portal-alpha draw blocks (~257–265 and ~285–293); densify the phase machine’s skip handling (`PH_ENTER` / `PH_MARCH` / shard all `finish()` on skip). Skip destinations and dialogue wrapping are visual + flow.
...
- **Horn chevron** in [src/combat.ts](../src/combat.ts) (~55 LOC of unique trig, `strokeChevron`, sprite-front/box-back midpoint). Replace with a simple facing AABB flash (the hitbox already exists). High unique-token tax. **Damage stays.**
- **Diagonal facing grace** in [src/player.ts](../src/player.ts) (~45 LOC, `DIAGONAL_RELEASE_MS = 100`). Horn facing becomes immediate last-axis; **movement itself unchanged**, but horn aim on diagonal release **will feel different**.
- **Heal `+` float** (~15 LOC in combat draw) and/or **floating damage numbers** in [src/fx.ts](../src/fx.ts) (~40 LOC). Keep pixel explosions (used everywhere → compresses). SPEC lists heal `+` as a juice item.
- Merge `openDeath`/`openWin` is in Cluster D (not a feel change).
...
1. **Static opening cutscene** (SPEC fallback #1) — **~400–700 B**
   - Keep the 3 dialogue panels + skip. Delete `PH_ENTER` / `PH_MARCH` (portal fade, boss walk, 7 walkers, timed pipe reveal/drain).
   - Bonus: delete [src/pipes.ts](../src/pipes.ts) `greyTwin` / `greyPipeCanvas` / grey kit bake (cutscene-only).
   - Loss: the “business men steal the rainbow” animation. World still starts colored, then greyscale on start.

2. **Straighten pipes** (SPEC fallback #4) — **~300–600 B** (Fable also listed ~100–180 for a milder straighten)
   - Replace `buildPipe` snake + occupancy grid + self-collision (~180 LOC) with one spoke: straight → one elbow → straight → cap. Keep `bakeKit`, accent elbows, portals, `takePipeSegment`.
   - Loss: organic S/C pipes. World still has 7 colored pipes to hunt.

3. **Instant color unlock** (SPEC wave fallback) — **~150–350 B**
   - Delete `snapshotTiles` / `tileCanvasesPrev` / clip-arc double `drawTiles` / expanding `colorWave`. Unlock + short flash (reuse explosion or a 1-frame fill).
   - Loss: radiating ground wave. Sprites already pop to color instantly today (wave is tiles-only).

4. **Simpler miniboss death** — **~80–200 B** (related Fable item: pipe-death explosion spam reduction ~50–100)
...
5. **Shorter placeholder song** — **~50–150 B** (music still loops). Fable’s larger band for cutting the track + `zzfxm.ts`: **~600–900 B**.
   - 6 patterns share identical drum/bass rows; fewer patterns / shorter sequence in [src/music.ts](../src/music.ts).
   - Not dropping ZzFXM unless this is explicitly chosen as a last-resort audio-reservation cut (SPEC ladder #5).

6. **Horn chevron VFX, keep damage** — **~180–280 B** (same as Track 1 Cluster E; listed here if deferred).

7. **Pipe accent-continuity** (inner/outer curve kits; occasional rim seams) — **~200–300 B**
   - Grok treated accent elbows as load-bearing; Fable listed this as a cut. It is a **visual** cut, not golf. Late rung.

8. **Floating damage numbers / heal `+`** — **~100–170 B** / ~15 LOC (same as Cluster E).

9. **Shorter dialogue / UI / unlock strings** — **~100–200 B**. Unique strings compress worst. Content decision, not golf.

### Meta / systems (keep combat, thin the between-run layer)

10. **Drop shop rows XP GAIN + SCRAP GAIN** (SPEC fallback #3) — **~120–200 B**
    - Least-felt rows (meta-on-meta drop chance). Shop UI stays. [src/stats.ts](../src/stats.ts) `SHOP_ROWS` 11→9.

11. **Drop LUCK row** — **~120–180 B extra**
    - Also deletes `LUCK_FOURTH` / `LUCK_FIFTH` and the 4th/5th card roll in `dealLevelUpCards`. Hands are always 3 cards.

12. **Drop START SPD** — **~50–80 B**
    - Overlaps yellow speed. Keep in-run yellow + shop Start HP / stats / magnet / revive.

13. **Final boss 2–3 powers instead of 6** — **~100–160 B**
    - SPEC allows dropping finale heal/shield if the fight drags. Small bytes. Not the same as cutting a player power.

14. **Cut frostball or shield power** — **~120–250 B**
...
Shop XP/Scrap rows → instant wave → static cutscene → straighten pipes → luck row.
